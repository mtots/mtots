...
struct Symbol {
  ...
  /** a cached Str - since Symbol should not know about Values and Objects,
   * we just have a void pointer here to be used with Str */
  void *str;
};
...
static u32 hashStringData(const u8 *key, size_t length) {
...
Symbol *internStringWithLength(const char *chars, size_t length) {
  return intern((const u8 *)chars, length, hashStringData((const u8 *)chars, length));
}

Symbol *internSymbol(const char *cstring) {
  return internStringWithLength(cstring, strlen(cstring));
}
